...
def install_modules():
    ...
    if ret.stdout.strip():
        process_module_output(ret.stdout.strip().split("\n"))
        user_manifests_path = os.path.join(".", "resources", "user", "meta", "manifests")
        if not os.path.isdir(user_manifests_path):
            os.makedirs(user_manifests_path)

        with open(os.path.join(user_manifests_path, "settings.json"), "w") as settings:
            settings.write(
                json.dumps(
                    {
                        "py": args,
        ...
        with open(os.path.join(user_manifests_path, "pipline.txt"), "w") as settings:
